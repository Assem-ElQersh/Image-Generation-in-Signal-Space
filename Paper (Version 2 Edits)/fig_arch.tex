...
% Left column: conditioning, aligned with transformer
% =========================================================
\node[txtbox, left=18mm of tx.west, anchor=east] (cond) {cond.\ vector $\mathbf{c}$};
\node[txtbox, above=of cond]                      (clip) {CLIP ViT-B/32};
\node[txtbox, above=of clip]                      (txt)  {prompt};

\draw[arr] (txt.south)  -- (clip.north);
\draw[arr] (clip.south) -- (cond.north);

% Conditioning -> transformer (FiLM) — straight horizontal arrow
\draw[arr] (cond.east) --
    node[midway, above, font=\sffamily\scriptsize]{FiLM}
    (tx.west);
